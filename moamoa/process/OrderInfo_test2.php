...
if($rs["ItemName"] == "명함") {
    ...
    if($rs["ProductName"] == "일반명함") {
        if($rs["PaperName"] == "스노우지 백색(무코팅) 250g") {
            $rs["PDFPath"] .= "/무코팅/" . $rs["stan_name"] . "/" . $rs["Tmpt"] . "도";
        }

        if($rs["PaperName"] == "스노우지 백색(무광코팅) 250g") {
        ...
        if(strpos($rs["PaperName"],"플러스") !== false) {
            $rs["Side"] *= 2;
        }
    }
    $rs["PDFPath"] .= "/" . $rs["QuantityName"];
} else if($rs["ProductName"] == "합판전단지") {
    if($rs["PaperName"] == "아트지 백색 90g" && $rs["SizeCode"] != "2절") {
        $rs["PDFPath"] = "/합판전단지";
        if($rs["SizeCode"][0] == 'A') {
            $rs["PDFPath"] .= "/국전지/";
        } else {
...
if($param['isaccept'] == 'Y') {
    ...
    $rs["ProductName"] = explode(" / ", $rs["Detail"])[1];
