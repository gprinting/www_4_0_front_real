...
if ($dvs === "memb") {
    ...
    $rs = $rs->fields;
    // 상민 추가 구절
    if($rs===null || $rs==="" || trim($rs)===""){
        //print_r($session);
        $rs = array();
        $rs["name"] = $session['name'];
        $cell_num = explode("-",$session['cell_num']);
        $tel_num = explode("-",$session['tel_num']);
        $rs["cell_num1"] = $cell_num[0];
        $rs["cell_num2"] = $cell_num[1];
        $rs["cell_num3"] = $cell_num[2];
        $rs["addr"] = $session['addr'];
        $rs["addr"] = $session['addr'];
        $rs["addr_detail"] = $session['addr_detail'];
        $rs["zipcode"] = $session['zipcode'];
        $dlvr_name = $recei = $session['name'];
    }
