...
for ($i = 0; $i < $after_info_arr_count; $i++) {
    ...
    if ($after_name === "오시" || $after_name === "도무송오시"
        ...
        while ($rs && !$rs->EOF) {
            ...
            if ($depth2 === "중앙" ||
                ...
                $html_arr[$after_name]["multi_manual"][$depth1 . "_" . $depth2] = $mpcode;
            } else {
                $html_arr[$after_name]["custom"] = $mpcode;
                $html_arr[$after_name]["multi_custom"][$depth1 . "_" . $depth2] = $mpcode;
            }
    ...
    } else if ($after_name === "라미넥스") {
        while ($rs && !$rs->EOF) {
            $fields = $rs->fields;
            $depth2 = $fields["after_name"];
            $mpcode = $fields["mpcode"];
...
foreach ($html_arr as $after_name => $info_arr) {
    $json .= sprintf($json_base,
                     ...
                     $info_arr["manual"],
                     $info_arr["custom"],
                     json_encode($info_arr["multi_manual"],JSON_UNESCAPED_UNICODE),
                        json_encode($info_arr["multi_custom"],JSON_UNESCAPED_UNICODE));
    $json .= ',';
}
