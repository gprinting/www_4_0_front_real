<?php
    ...
    if ( $res_cd == "0000" ) {
        ...
        if ($r_pay_type === "11") {
            ...
            $param["prepay_price_card"] = $r_amount;
            ...
            if ($conn->HasFailedTrans() === true) {
                ...
                $conn->RollbackTrans();
                $bDBProc = false;
                $debug_code = "D" + __LINE__;
                $debug_msg  = "회원 선입금액 수정에 실패했습니다.";
                goto OUT;
            }
            ...
            $param["depo_price"]      = '0';
            $param["card_depo_price"] = $r_amount;
            ...
            $param["card_cpn"]  = $r_acquirer_nm;
            $param["aprvl_num"] = $r_auth_no;
            ...
            if ($proc_ret === false) {
                ...
                $bDBProc = false;
                $debug_code = "D" + __LINE__;
                $debug_msg  = "회원결제내역카드 입력에 실패했습니다.";
                goto OUT;
            }

            $conn->CompleteTrans();

            // 일 매출 통계 테이블 값 수정(입금액)
            /*
            unset($param);
            $timeStrap             = date('Y-m-d');
            $param["modi_price"]   = $r_amount;
            $param["member_seqno"] = $member_seqno;
            $param["deal_date"]    = $timeStrap;
           
            $proc_ret = $dao->modiDepoDaySalesStats($conn, $param);

            if ($proc_ret === false) {
                $conn->FailTrans();
                $conn->RollbackTrans();
                $debug_code = "D" + __LINE__;
                $debug_msg  = "일 매출 통계 수정에 실패했습니다.";
                goto OUT;
            }
            */
        }
