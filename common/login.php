...
include_once(INC_PATH . "/com/nexmotion/doc/front/common/common.inc");
...
if (empty($seqno)) {
    ...
    $rs = $dao->selectMember($conn, ["id" => $id,"pw" => $pw]);
...
$gubun2 = $rs->fields["member_name"];
$gubun = $rs->fields["sell_channel"];
if($rs->fields["sell_channel"] == "" || $rs->fields["sell_channel"] == null) {
    $gubun = "GP";
}


if($rs->fields["sell_channel"] == $gubun){
...
if(empty($isadmin)) {
    ...
    echo sprintf($ret, $success, $referer, $_SERVER["SELL_SITE"], $gubun , $gubun2 );
} else {
    ...
    if($mode == "order") {
        ...
        header("Location: " . $url . "/product/esti_sheet.html?cs=010001001");
    } else if (empty($updateorder)) {
        header("Location: " . $url . "/main22/main.html");
    } else {
        ...
        header("Location: " . $url . $page . "?cs=" . $cate_sortcode . "&order_common_seqno=" . $updateorder);
    }
}
