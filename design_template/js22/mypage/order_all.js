...
var orderSearch = function (listSize, page) {
    ...
    var url = "/ajax22/mypage/order_all/load_order_list.php";
    var blank = "<tbody name=\"order_list\"><tr><td class=\"noResult\" colspan=\"" + colspan + "\">검색 된 내용이 없습니다.</td></tr></tbody>";
    var data = {
        "from": $("#from").val(),
        "to": $("#to").val(),
    ...
    var callback = function (result) {
        ...
        if ($("#im").val() == "1") {
            ...
            if (rs[0].trim() == "") {
                ...
                $("#paging").html(rs[1]);
                //$("#resultNum").html(rs[2]);
                $("#resultNum").html(rs[2] + ' <span style="color:#ff2222;">(※ 카드결제 주문취소건은 카드전표 클릭해서 결제취소 확인하세요.)</span>');
