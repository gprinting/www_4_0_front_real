...
$(document).ready(function () {
    ...
    $("#ad_size_name").val(order_stan_name);
    if(order_cut_wid != "" && order_cut_vert != "") {
        $("#ad_cut_wid_size").val(order_cut_wid);
        $("#ad_cut_vert_size").val(order_cut_vert);
    ...
    if (sortcode === "001004001") {
        ...
        $(prefix + "bonding").prop("checked", true);
...
*/
    ...
    {
        getAfterPrice.common('coating', dvs);
    }
...
var impressShowHide = function (dvs) {
    ...
    if (basisweight < 250) {
        ...
        if($(prefix + "thomson_impression").prop("unchecked"))
            $(prefix + "thomson_impression").trigger("click");
...
var click_after_btn = function() {
    if(sortcode == "001002001" && folding_warning == false) {
        alert("리플렛/팜플렛 제품은 접지가 기본으로 들어갑니다.");
        folding_warning = true;
    }
}
